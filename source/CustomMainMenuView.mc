...
class CustomMainMenuView extends WatchUi.View {
    ...
    // Kept in sync with the (:bigGames) annotation - see Games.HAS_BIG_GAMES.
    private const BIG_GAMES = [:item_chess, :item_checkers];
    ...
    function initialize() {
        ...
        for (var i = 0; i < labelIds.size(); i++) {
            ...
            if ((touch || TOUCH_ONLY_GAMES.indexOf(Games.IDS[i]) < 0) &&
                ...
                (Games.HAS_BIG_GAMES || BIG_GAMES.indexOf(Games.IDS[i]) < 0)) {
                _items.add(i);
            }
        }
        sortItems(_items);
